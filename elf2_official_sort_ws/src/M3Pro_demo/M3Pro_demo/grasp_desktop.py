...
class GraspNode(Node):
	def __init__(self, name):
		...
		self.pubBlockHeld = self.create_publisher(Bool,"block_held",latched_bool_qos())
		self.publish_block_held(False)
		...
		self.declare_parameter("gripper_close", 165)
		self.declare_parameter("grasp_x_delta", 0.01)
		self.declare_parameter("grasp_y_delta", -0.007)
		self.declare_parameter("grasp_z_delta", -0.001)
		self.declare_parameter("hold_after_pick", False)
		self.joint6 = self.get_parameter("gripper_close").get_parameter_value().integer_value
		self.grasp_x_delta = self.get_parameter("grasp_x_delta").get_parameter_value().double_value
		self.grasp_y_delta = self.get_parameter("grasp_y_delta").get_parameter_value().double_value
		self.grasp_z_delta = self.get_parameter("grasp_z_delta").get_parameter_value().double_value
		self.hold_after_pick = self.get_parameter("hold_after_pick").get_parameter_value().bool_value
		self.get_logger().info(
			f"grasp deltas x={self.grasp_x_delta:.3f} y={self.grasp_y_delta:.3f} z={self.grasp_z_delta:.3f} "
			f"gripper_close={self.joint6} hold_after_pick={self.hold_after_pick}"
		)
	...
	def grasp(self,pose_T):
		...
		request.tar_y = pose_T[1] + self.grasp_y_delta
		request.tar_z = pose_T[2] + self.grasp_z_delta
		request.tar_x = pose_T[0] + self.grasp_x_delta
	...
	def move(self):
		...
		time.sleep(2.5)
		if self.hold_after_pick:
			self.pubSixArm([90, 180, 0, 0, 90, self.joint6], runtime=2000)
			time.sleep(2.5)
			self.grasp_flag = True
			self.publish_block_held(True)
			print("block picked and held; waiting for delivery node")
			return
		if self.cur_tagId == 1:
	...
	def publish_block_held(self, held):
		block_held = Bool()
		block_held.data = bool(held)
		for _ in range(5):
			self.pubBlockHeld.publish(block_held)
			time.sleep(0.1)
